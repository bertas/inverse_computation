...
main :-
    ...
    %lose(InStream), 
    open(FileName, read, InStream2),
    %time(rF_exec(1000, FileName, OutStream)),
	read_file(InStream2, OutStream, _), 
    close(InStream2), 
    close(OutStream).

rF_exec(0, _, _).
rF_exec(Times, FileName, Out):-
    open(FileName, read, In),
    read_file(In, Out, _),
    close(In),
    NTimes is Times -1,
    rF_exec(NTimes, FileName, Out).
...
read_file(InStream, OutStream, [Clause|TrailingClauses]) :-
	...
	NewClause =.. NewClauseList, numbervars(NewClause, 0, _N),
