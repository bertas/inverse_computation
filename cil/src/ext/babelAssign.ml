...
class babelAssign f =
  ...
  (* let babelFuncWhiteList = ["init64"; "dec64"; "enc64"] *)
  (* let babelFuncWhiteList = ["encode"; "decode"; "sum"; "md5"; "init64"; "dec64"; "enc64"] *)
  ...
  let e_size e =
    ...
    let rec size_m t =
        ...
        | TFloat (fk, _) ->
          begin
            match fk with
            (* FIXME : I need to specifically judge double and float, so. *)
            | FDouble -> 8
            | FFloat -> 4
          end
...
object (self)
  ...
  method update_whitelist =
    ...
    let read_file filename =
      ...
      with End_of_file ->
        close_in chan;
        List.rev !lines in
    let il = read_file "babel_whitelist_new.txt" in
  ...
  method mkAssign lv e =
    ...
    in
      (* "babelAssign("^(String.uppercase (lv_str lv))^", "^( String.uppercase *)
      (* (e_str e))^")," *)
  ...
  method singleArg count arg =
      (* here is the tricky thing,
        some of the argments are constant and don't have name,
        so eventually we generate random name for each variables
      *)
  ...
  method ret lv =
     ...
     | TVoid _ -> ""
     |  _ -> t_str ty
  (*TODO : do we need to adjust this part*)
...
(* ingore functions begin with babel_wrapper_ *)
  ...
  method is_logic e =
    (* this is wired, the type of (a == 1) in the above example is **int** *)
    let help_t t =
      match t with
       | TInt (ik,_) ->
          begin
...
(* here is the case, it is somehow hard to translate this kind of exp
 *        int a = 1 + b + array[i];
 * so we reply on a script (pre_process.py) to pre process this instruction into
 *        int array_temp = array[i];
 *        int a = 1 + b + array_temp;
 *
 * same situation can also use to solve this issue
 *       int a = 1 + b + *p;
 *       ====>
 *       int ptr_temp = *p;
 *       int a = 1 + b + ptr_temp;
 *
 * then based on this design, now the exp could only be
 *   1. const
 *   2. lval
 *       * normal lval : we go normal babelAssign way
 *       * array  : babel_array_get
 *       * ptr    : baebl_ptr_get
 *   3. .....  normal cases
 * *)
 ...
 * usually the ptr artih operation cases is something like this:
 *   ptr2 = ptr1 + int_val;
 *   then we translate it into
 *   ptr2 = ptr1 + TypeSizeOf(ptr1) * int_val;
 *
 *   It is quite rare to find more than one pointer var in the exp
 *   in that case, we fail the process
 *   *)
  ...
  method pointer_r_interface l =
    ...
    | "2" ->
      "babelPtrR"
    | "1" -> "babelPtrR_byte"
    (*FIXME:  when length is 1, there should be byte or char two possibilities*)
  ...
  method trans_instr i =
   ...
   match i with
    ...
    (* based on C-To_Prolog translation, we can simply ignore this situation here *)
        (* failwith "trans instruction found pointer init" *)
        ""
    (* *ptr1 = *ptr2;  *)
  ...
  *  loop_entry(N, A, ResultN, ResultA) :-
    ...
    TODO: Currently this is the most suitable way I can figure out,
    perhaps we can find better tranform solution on loop.
  * *)
  method checkGoto b1 =
  ...
  method getAllVariablesInst i acc =
    ...
    (* TODO : other kind of instructions*)
    | _ -> []
...
let feature : featureDescr =
  { fd_name = "BabelAssign";
    ...
    fd_description = "transform a program's assign instruction in Prolog";
    fd_extraopt = [];
    fd_post_check = true;
    fd_doit =
      function (f: file) ->
